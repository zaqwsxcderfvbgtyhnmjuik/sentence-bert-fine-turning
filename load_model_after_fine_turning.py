...
class ModelLoader(nn.Module):
    def __init__(self, modelName):
        ...
        if self.modelName == "SentenceBert":
            self.tokenizer= AutoTokenizer.from_pretrained('E_Post2Github/model')     # Note: if you wanna Load model from HuggingFace Hub, set the value of argument as following:
            self.model = AutoModel.from_pretrained('E_Post2Github/model')            # 'sentence-transformers/paraphrase-multilingual-mpnet-base-v2'
...
def get_sentences_embedding(sentences = ["completeness cheak", "过程设计"]):
    model = torch.load(r"model_after_fine_turning/model_after_fine_turing.pt")
    model.eval()
